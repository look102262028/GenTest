...
public class fortifyConversionMain {

	
	static String basePath=System.getProperty("user.dir")+File.separator;
	public static String inputFile = basePath+"test2.pdf";	//D:\\workspace\\Mobile\\GenTest\\test.pdf
	public static String inputFile1 = basePath+"test_samerow2.pdf";	//D:\\workspace\\Mobile\\GenTest\\test_samerow.pdf
	...
	public static void main(String[] args) throws IOException, DocumentException {
		...
		if(!"1".equals(sameROW)) {	//兩行
			new fortifyConversionMain().manipulatePdf(inputFile, outputFile, map);
		}else {	//BuildID一行
	...
	public void manipulatePdf(String src, String dest, Map<String, String> map) throws IOException, DocumentException {
		...
		System.out.println("123:"+map.get("day"));
...
//		// 1.檢測日期:2023年4月25日，檢測主機名稱:SC-Fority-02，檢測專案代號
		float[] result1 = PdfConversion.getKeyWords(src, "SC-Fortify");
	...
	public void manipulatePdfSameRow(String src, String dest, Map<String, String> map) throws IOException, DocumentException {
		...
		PdfStamper stamper = new PdfStamper(reader, new FileOutputStream(dest));
		// 0.日期
		float[] result = PdfConversion.getKeyWords(src, "2023/11/28");
		PdfContentByte canvas = stamper.getOverContent((int) result[2]);
		float x, y;
		x = result[0];
		y = result[1];
		...
		float[] result3 = PdfConversion.getKeyWords(src, "：312");
		PdfContentByte canvas3 = stamper.getOverContent((int) result3[2]);
		float x3, y3;
		x3 = result3[0];
		y3 = result3[1];
